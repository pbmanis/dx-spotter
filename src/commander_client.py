...
``commander_port`` setting in ``config.toml`` or pass ``port=`` to
:class:`CommanderClient` to match your installation.
...
_DEFAULT_PORT: int = 52002   # Commander's documented default (base port + 2)
_TIMEOUT: float = 0.2
_VERIFY_DELAY: float = 0.75   # minimum ~0.7 s to cover one Commander rig-poll cycle
...
class TransceiverState:
    ...
    mode : str
        Active transceiver mode, e.g. ``'CW'``, ``'USB'``, ``'LSB'``.
    split : bool
